/** What sends the ball behind for corners, and what happens from them. */
...
for (let seed = 1; seed <= n; seed++) {
  ...
  for (const e of state.events) {
    if (e.type === 'pass') last = `pass${e.lofted ? ' lofted' : ''}`
    else if (e.type === 'clearance') last = `clearance${e.header ? ' header' : ''}`
    else if (e.type === 'deflection') last = `deflection ${e.kind}`
    else if (e.type === 'shot') last = `shot${e.header ? ' header' : ''}`
    else if (e.type === 'possession') last = 'possession'
    else if (e.type === 'out' && e.award === 'corner') out[last] = (out[last] ?? 0) + 1
  }
}
console.log(Object.entries(out).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k}: ${(v / n / 2).toFixed(2)}`).join('\n'))
